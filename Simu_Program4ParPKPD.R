...
simu_gfun4parPKPD <- function(n=1000, dose=1, ka=9, gamma=3, c50=0.4, delta=0.1, sig=20){
...
fit.pd <- optim(par0, likpd.dose, data=df_E, diclst=dic.pd
                ,fun=gfunE, method="BFGS", hessian=TRUE
...
lik = -likpd.dose(est,data=df_E,diclst=dic.pd,fun=gfunE)

### show AIC, BIC
aic = -2*lik+2*length(est)
bic = -2*lik+log(nrow(df_E[df_E$event==1,]))*length(est)

## Naive SE
seN = sqrt(diag(solve(H)))

## Robust SE allowing for multiple events within person
SEs <- Robust.error(est,Hess=H,data=df_E,doseX=doseX,fun=gfunE)
seR <- sqrt(diag(SEs$ClusterH))

## peak information
peakV <- peak.information(est,doseN=dose)

## true values
trueV <- c(dose,log(gamma),log(ka),delta,log(c50))

#return all values
c(n,N0,N1,trueV,est,seN,seR,aic,bic,peakV)
